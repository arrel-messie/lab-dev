...
<body>
    ...
    (function() {
      var notesSectionSelector = '.notes-section';
      var notesHistory = [];
      var notesFuture = [];
      var currentNote = null;

      function renderNote(notePath, pushHistory) {
        if (pushHistory === undefined) pushHistory = true;
        var notesSection = document.querySelector(notesSectionSelector);
        if (!notesSection) return;
        var notesContent = notesSection.querySelector('.notes-content');
        if (!notesContent) return;
        var html = window.allNotes[notePath] || '<em>Note introuvable.</em>';
        ...
        var links = notesContent.querySelectorAll('.note-link');
        for (var i = 0; i < links.length; i++) {
          links[i].addEventListener('click', function(e) {
            e.preventDefault();
            var target = this.getAttribute('data-note');
      ...
      function renderNotesNav() {
        var notesSection = document.querySelector(notesSectionSelector);
        if (!notesSection) return;
        var nav = notesSection.querySelector('.notes-nav');
        if (!nav) {
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        var items = document.querySelectorAll('.description-card.has-notes');
        for (var i = 0; i < items.length; i++) {
          items[i].addEventListener('click', function() {
            var notesSection = this.closest('.description-card[data-category]').querySelector('.notes-section');
            if (notesSection) {
              var cat = this.closest('.description-card[data-category]');
              if (cat) {
                var catName = cat.getAttribute('data-category');
                var notePath = null;
                for (var k in window.allNotes) {
        ...
        var defaultLinks = document.querySelectorAll('.notes-section .note-link');
        for (var i = 0; i < defaultLinks.length; i++) {
          defaultLinks[i].addEventListener('click', function(e) {
            e.preventDefault();
            var target = this.getAttribute('data-note');
            if (target) renderNote(target);
          });
